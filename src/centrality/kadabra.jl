...
function kadabra_centrality(
    ...
    if parallel
        ...
        # Pairs drawn per worker, updated after every sample. A worker's counts grow with
        # each sample while `n_pairs2` only grows per completed batch, so a check must use
        # these to divide the counts it reads by the number of pairs they come from. Each
        # slot is its own heap object so per-sample writes do not share a cache line.
        pair_slots = [Threads.Atomic{Int}(0) for _ in 1:nthreads]
                ...
                Threads.@spawn begin
                    ...
                    while !stop_flag[] && n_pairs2[] < omega
                        for _ in 1:check_interval
                            # Stop within one sample of another worker's successful check
                            # rather than finishing the batch.
                            stop_flag[] && break
                            s = rand(t_rng, 1:n)
                            t = rand(t_rng, 1:n)
                            while s == t
